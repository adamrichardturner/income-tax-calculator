function getTax() {
  ...
  var total; // total income after adding two fields in HTML
  var afterTax; // net income after tax
  var taxTaken; // tax contribution
  var income = +document.getElementById('income-work').value;
  var income_other = +document.getElementById('income-other').value;
  var frequency = document.getElementById('frequency').value; // The calendar interval e.g. 1 = Annual
  ...
  if (!isNaN(income) && !isNaN(income_other)) {
    total = income + income_other;
      if (total > 0 && total <= personal_allowance_top && frequency === '1') {
        ...
        taxTaken = (basic_rate_top - basic_rate_bottom) * basic_rate_percent + (total - higher_rate_bottom - personal_allowance_top) * higher_rate_percent;
        afterTax = total - taxTaken;
        frequency_declared.innerHTML = ("Annual Income and Tax");
        income_marker.innerHTML = ("£" + total);
        income_after.innerHTML = ("£" + afterTax);
